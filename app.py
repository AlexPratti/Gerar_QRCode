...
def process_pdf_adaptativo(file, tipo_selecionado):
    data = []
    # Se o rádio estiver em DIVERSOS, ele só aceita essa categoria
...
doc_ativo = st.radio(
    "Qual arquivo deseja visualizar ou atualizar?",
...
with st.expander("⬆️ Gerenciar e Upload de Arquivos"):
    # REATIVADO: Agora aceita vários arquivos novamente
    ...
    if arquivos_carregados:
        ...
        arquivo_escolhido_nome = st.selectbox("Escolha qual destes arquivos aplicar à opção selecionada no rádio:", lista_nomes)
        
        if st.button(f"Sincronizar {arquivo_escolhido_nome} como {doc_ativo}"):
            with st.spinner("Processando..."):
                # Busca o objeto do arquivo correto na lista de carregados
                arq_obj = next(f for f in arquivos_carregados if f.name == arquivo_escolhido_nome)
                file_bytes = arq_obj.read()
                
                # Salva no Storage com o nome padrão (LITÚRGICOS ou DIVERSOS)
                supabase.storage.from_(BUCKET).upload(path=NOME_STORAGE, file=file_bytes, file_options={"x-upsert": "true"})
                
                # Processa os dados
                dados = process_pdf_adaptativo(io.BytesIO(file_bytes), doc_ativo)
                save_to_db(dados)
                st.success(f"O arquivo {arquivo_escolhido_nome} agora está ativo como {doc_ativo}!")
                st.rerun()
...
try:
    ...
    if res_cat.data and arquivo_persistente:
        ...
        if res_hinos:
            ...
            hino_sel = st.selectbox("Escolha o Hino:", [h['nome_nivel2'] for h in hinos_ord], key=f"h_{escolha_n1}")
            ...
            with pdfplumber.open(arquivo_persistente) as pdf:
                ...
                for l in text_lines:
                    if l['top'] > y_ini + 5:
                        if re.match(r'^\d+\.', l['text'].strip()) or l['text'].strip().upper() in bloqueio:
                            y_fim = l['top']
                            break
